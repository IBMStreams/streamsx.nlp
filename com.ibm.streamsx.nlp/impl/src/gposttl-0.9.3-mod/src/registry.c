...
//     unsigned int size_hint_value;
{
  ...
  lower(dir)->hash_table = 
    (RegistryRecord **)Memory_allocate(sizeof(RegistryRecord *)
				   * size_hint_value);
  for (i=0; i < (int) size_hint_value; ++i)
...
//     VOIDP priv_ptr;
{
  ...
  for (i = 0; i < (int)lower(dir)->ht_size; ++i)
...
//     unsigned int htsize;
{
  ...
  if ((long)uns_int_ptr > 0)
    return htsize ? (long)uns_int_ptr % (long)htsize : 0;
  else
...
int Registry_strcasecmp(CONSTVOIDP str1, CONSTVOIDP str2)
//     CONSTVOIDP str1;
//     CONSTVOIDP str2;
{
